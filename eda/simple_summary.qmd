...
Every year, the **North Carolina Rating Bureau**^1^ publishes the Workers Comp Loss Costs by class of business. Insurers use these loss costs to form the rates for Workers Compensation insurance policies.

> *1: North Carolina is a bureau state, and does not primarily use NCCI like most other states.*

::: callout-note
Note, this analysis is presented w/out warranty and was not produced by an actuary.
:::

### Simple Trends in Loss Costs

These initial views track the change in approved loss costs over time.

> "Pursuant to the provisions of Article 36, Chapter 58 of the General Statutes of North Carolina, enclosed is the filing for workers compensation insurance prospective advisory loss costs, rating values and miscellaneous values applicable to new and renewal policies becoming effective on or after April 1, 2023."

Typically, in this current market cycle, the frequency of workers compensation claims is decreasing at a consistent clip, while the severity of claims is flat to only slightly increasing.  As a result, loss costs are on a long-term downward trend. This is the case in both North Carolina and in almost all states. 

The frequency times the severity is the loss cost. Most class codes have a loss cost between \$1 and \$2.50 which is multiplied by every \$100 of payroll to determine the manual premium. For many white-collar office jobs, loss costs are frequently well less than 25 cents. 

F-Class classes are high-hazard classes that are atypical and uncommon.
Loss costs declined precipitously in 2019 and in 2020, but the decreases have moderated substantially since then.
...
# load a rds file
interim <- stringr::str_sub(getwd(), end = -5L)
loader_path1 <- paste0(interim, "/etl/ingot", "/dataframe.rds")
clockin()
...
df %>% mutate(lc_cap = ifelse(loss_cost > 10, 10, loss_cost)) %>% 
  # filter(loss_cost < 20) %>% 
...
### Focusing on the most recent YoY changes

These views will focus on the change in loss costs from the most recent year to the upcoming year.
...
-   <https://www.ncrb.org/ncrb/workers-compensation/circulars>

-   [Rate and Filings](https://www.ncrb.org/ncrb/workers-compensation/rate-filings)

Packages primarily used in this analysis: `library(lobstr)  library(rlang)  library(tidyverse)  library(tidylog)  library(scales)  library(gt)  library(patchwork)`
